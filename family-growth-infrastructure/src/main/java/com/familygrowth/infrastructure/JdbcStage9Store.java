...
class JdbcStage9Store implements Stage9Store {
    ...
    @Override
    public UsagePolicy upsertPolicy(UUID family, UUID child, String zone, int daily, int session, UUID actor, Instant now) {
        requireChild(family, child);
        int updated = jdbc.update("""
            UPDATE usage_policy SET zone_id=?,daily_limit_minutes=?,session_limit_minutes=?,version=version+1,
            actor_id=?,updated_at=? WHERE family_id=? AND child_id=?
            """, zone, daily, session, actor, ts(now), family, child);
        if (updated == 0) {
            try {
                jdbc.update("""
                    INSERT INTO usage_policy(child_id,family_id,zone_id,daily_limit_minutes,session_limit_minutes,version,actor_id,created_at,updated_at)
                    VALUES(?,?,?,?,?,0,?,?,?)
                    """, child, family, zone, daily, session, actor, ts(now), ts(now));
    ...
    public UsagePolicy policy(UUID family, UUID child) {
        ...
        return jdbc.query("SELECT * FROM usage_policy WHERE family_id=? AND child_id=?", this::policyRow, family, child)
            .stream().findFirst().orElse(new UsagePolicy(family, child, "Asia/Shanghai", 20, 10, 0, Instant.EPOCH));
    }
    ...
    private UsagePolicy policyRow(ResultSet r, int n) throws SQLException { return new UsagePolicy(uuid(r, "family_id"), uuid(r, "child_id"), r.getString("zone_id"), r.getInt("daily_limit_minutes"), r.getInt("session_limit_minutes"), r.getLong("version"), instant(r, "updated_at")); }
