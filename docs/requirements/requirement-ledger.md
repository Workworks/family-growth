...
| REQ-039 | 2026-08-28 | 用户连续推进指令 | 进行下一步 | 按 TODO 进入 Stage 21 V12：加密持久化最小学习 outbox，保留原幂等键并在重新连接后恢复；401 保留队列等待家长登录，409 刷新服务端事实后显式合并，不静默覆盖或丢弃。家长端增加“一课一活动”的可视化建课、发布与分配纵切；足量课程、多课节自由编排和目标平板仍不冒充完成 | P0 | `VALIDATED` | 21 | `LearningOutbox.kt`、`TeachingStudioUi.kt`、课程版本读取 API、Android 双变体 38 项与 Stage 21 evidence；真机运行态仍归 AC21-06 |
