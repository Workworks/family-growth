...
| WP10-3 | 已完成 | 本地/CI 稳定签名 0.3.0 APK、GitHub Release、公开 digest 与下载复验 |
...
- Android 版本提升到 0.3.0/versionCode 6；workflow `32916602740` 全部成功并创建 [v0.3.0 Release](https://github.com/Workworks/family-growth/releases/tag/v0.3.0)。公开 APK 下载后复验包名、v2 稳定证书、大小和 GitHub digest，并作为 `dist` 最终副本。
...
- [x] AC10-03 `PASS`：debug/release、本地/CI 稳定签名、版本、包名、GitHub digest、公开 Release 和下载复验通过。
...
Stage 10 可离线工程与公开 Release 已收口；只有在真实设备与目标部署环境完成最终回放后才能 `COMPLETED`，因此状态为 `BLOCKED`。本报告的 AC-V1-11/13/17/19 记录 Stage 10 当时只有本机零钱回收、PIN/最小日志、App 内分钟限制和透明费用基础实现；后续 Stage 17 已补齐服务端冻结式兑现，Stage 26 已补齐自助数据权利、自动保留、睡眠时段、单次会话/强制休息和临时放行审计。当前能力以对应后续 Stage 证据为准，仍不得把自动化结果冒充目标平板/部署运行态验收。
