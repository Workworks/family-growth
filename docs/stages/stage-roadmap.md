...
| 22 | 全面教学幼儿园深度 | 幼儿园故事舞台与亲子现实活动 | `IN_PROGRESS` |
...
REQ-030/031 把全面教学确定为下一产品主线，并按幼儿园、小学、初中、高中重排。Stage 20 的 V9、服务/API、权利目录、四学段 Android 路由和全部离线门禁已完成，只缺目标平板运行态而保持 `BLOCKED`。Stage 21 的共用课程/活动/证据引擎、加密 outbox 和家长一课一活动工作台也已完成离线门禁，只缺目标平板/服务 E2E 而转为 `BLOCKED`。REQ-041 已建立 Stage 22 Spec 并启动幼儿园单一行动安全投影；Stage 23–26 仍是规划，建立对应 Spec 前不进入上方状态表。
