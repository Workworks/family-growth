...
| 23 | 全面教学小学深度 | 小学探索手册 | `IN_PROGRESS` |
...
REQ-030/031 把全面教学确定为下一产品主线，并按幼儿园、小学、初中、高中重排。Stage 20 的 V9、服务/API、权利目录、四学段 Android 路由和全部离线门禁已完成，只缺目标平板运行态而保持 `BLOCKED`。Stage 21 的共用课程/活动/证据引擎、加密 outbox 和家长一课一活动工作台也已完成离线门禁，只缺目标平板/服务 E2E 而转为 `BLOCKED`。REQ-041–043 已完成 Stage 22 WP22-1–5 的可离线纵切，包括幼儿园安全投影、服务端内容策略、10 个原创亲子活动和家长观察回应；目标平板未回放，故 Stage 22 也转为 `BLOCKED`。REQ-044 已启动 Stage 23 并完成 WP23-1/2 Android 可离线首切；Stage 24–26 仍是规划，建立对应 Spec 前不进入上方状态表。
