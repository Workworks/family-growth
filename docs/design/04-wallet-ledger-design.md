...
Wallet 保存可快速读取的当前余额：`money_balance`、`coin_balance`、`version`。LedgerEntry 是每次变化的不可变业务事实，至少包含：账户、资产类型、delta、before、after、entryType、业务引用、幂等键、操作者、原因、时间、关联流水组。
...
| 零钱回收/提现 | 申请前展示回收比例、固定/比例手续费和预计线下兑现额；`PAID` 时扣 Money 并记录 `WITHDRAWAL` 与 `WITHDRAWAL_FEE` 明细；APPROVED 阶段冻结对应 Money 防止重复支出 |
| 家长调账 | `PARENT_ADJUSTMENT`，强制原因、before/delta/after、操作者 |

## 并发、幂等与对账

- 每个业务动作接受 `Idempotency-Key`，并建立 `(family_id, operation_type, key)` 唯一约束。
- Wallet 用乐观锁/行锁防止超卖；余额不得为负（除非未来显式设计信用能力，V1 禁止）。
...
`netPayout = max(0, grossPayout - fee)`

这里的“手续费”是家长用于财商教育的家庭规则，不是平台作为真实金融中间商收款。确认页和流水必须分别显示申请 Money、比例、手续费、净兑现额和线下兑现声明；禁止把手续费藏入兑换比例。

## 安全不变量

孩子不能调用调账、费率、预算接口；所有接口同时校验 family/child 所属。费用预览与执行之间若规则版本、NAV 或金额变化，执行必须拒绝并要求重新预览。
