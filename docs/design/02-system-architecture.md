...
- 学习 Attempt/Submit/Review 采用本机写前 outbox：最小动作载荷先以 Android Keystore 256-bit AES/GCM 加密并同步提交到私有存储，再携带原 `Idempotency-Key` 请求服务端。队列不保存 PIN、Token、答案键或权利材料，最多 100 项；密文损坏或写入失败时 fail-closed。
- 进程重启只恢复加密待办，不恢复会话。家长重新连接同一家庭/孩子后按创建时间顺序重放；401 清除内存 Token 但保留待办，409 先刷新 Assignment 并按服务端版本/目标状态显式合并，不能离线生成 `MASTERED` 或奖励。
