...
| 共用教学引擎 | `teaching_course`, `_version`, `_unit`, `_lesson`, `learning_activity`, `_question`, `_question_option`, `lesson_assignment`, `activity_attempt`, `learning_completion`, `mastery_evidence`, `teaching_action` | 发布版本只读；九类活动固定证据规则；答案键服务端保存；Assignment 乐观版本与动作幂等；历史证据追加式 |
