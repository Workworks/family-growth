...
| 17 | V1 深度 | 服务端零钱回收与冻结式兑现闭环 | `IN_PROGRESS` | Stage 18 当前反馈已交付；恢复 V8 迁移、状态机、available 保护和 PostgreSQL 门禁 |
| 18 | V1 Android | 双视角排版、奖励浏览与教学视频任务 | `BLOCKED` | 离线代码与 v0.3.2 已交付；用户平板测试布局、播放、持久化和热更新 |
| 19 | 发布可靠性 | GitHub Release 下载 0% 真机修复 | `IN_PROGRESS` | 当前优先改造系统下载、阶段反馈、官方备用入口并发布两版测试链 |
