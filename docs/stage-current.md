...
| 8 | 7 | Android 生产接入 | `IN_PROGRESS` | 接入服务端会话、家庭/孩子和核心业务同步状态 |
...
REQ-020/REQ-022 已恢复 Stage 3–10 生产闭环。Stage 3–7 后端已完成；Stage 8 正在将 Android 从本机演示状态接入生产 API，后续不跨越联调门禁提前标记完成。

REQ-021 将“儿童最佳利益、3 岁起、极简低刺激、无操纵设计”提升为最高产品行为准则。Stage 15 可离线代码、13 项 JVM 测试、lint、debug/release 构建和稳定签名 0.2.2 APK 已通过；真实平板验收阻塞。Stage 8 现在是 Agent 主线。
