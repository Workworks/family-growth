...
| 21 | 全面教学核心 | 共用课程、活动与学习证据引擎 | `IN_PROGRESS` | V11 课程版本、九类活动、证据状态机和 Android 学习路径已过离线门禁；下一步做持久化 outbox、401/409/重启恢复和家长可视化建课，等待真实来源/平板回放 |
...
REQ-037/038 已交付 V11 离线纵切：服务端提供不可变课程版本、九类活动、题目答案隔离、课节分配、尝试/提交/复做/确认和五类证据；Android 在原“小任务”页渲染单课节学习路径，安装包视频实际播放 90% 只记 VIEWED，家长可确认或要求复做。H2/PostgreSQL 16.15 全量 46 项、11 次迁移/55 表、PostgreSQL 并发审核和 Android 两变体各 31 项通过。持久化 outbox、专门的 401/409/重启恢复、家长可视化建课和目标平板仍未完成，Stage 21 继续 `IN_PROGRESS`。
