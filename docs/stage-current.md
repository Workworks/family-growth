...
| 22 | 全面教学幼儿园深度 | 幼儿园故事舞台与亲子现实活动 | `IN_PROGRESS` | Android 单一行动投影与 V12 服务端年龄带/五领域发布门禁已通过；下一步制作首批原创亲子活动及家长模板 |
...
REQ-041 已按 TODO 最高顺位启动 Stage 22。Spec 将幼儿园深度定义为 3–5 岁亲子现实活动和家长回应，不是增加屏幕时长；首个 Android 切片按学段移除课程轮播，只显示当前活动，并对超过 8 分钟、超过两个选择或缺短指令的内容安全停止。服务端内容带已由 REQ-042 补齐，首批原创内容和目标平板验收尚未完成，Stage 22 保持 `IN_PROGRESS`。

REQ-042 已完成 WP22-3：V12 将幼儿园 `SHARED_3_4/TRANSITION_5_6` 内容带和健康/语言/社会/科学/艺术标签随不可变课程版本保存；发布用例校验每课活动数、总时长、屏幕时长、选择数和亲子/离屏出口。H2 全量与 PostgreSQL 16.15 全量通过；首批原创内容和家长模板仍未完成，因此 Stage 22 继续为 `IN_PROGRESS`。
