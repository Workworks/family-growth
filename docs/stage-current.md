...
| 21 | 全面教学核心 | 共用课程、活动与学习证据引擎 | `IN_PROGRESS` | V10 家长免费来源/动态栏目已过离线门禁；继续 V11 课程版本、九类活动与证据链，等待真实来源/平板回放 |
...
REQ-034/035 将孩子自主学习和国家中小学智慧教育平台同步课堂提升为当前 Stage 21 首个 Android 纵切。小学/初中/高中现已在既有学习入口获得“我的任务/自主学习”切换、家长学段锁定、教材五级选择和受限官方 WebView；两变体各 28 项 JVM、lint/build 通过。官方页面已核验存在对应筛选；产品虽仅家庭自用，本轮仍不抽取视频流、不修改页面、不将第三方播放冒充完成证据。目标平板播放、站内路径和无障碍仍未回放，Stage 21 保持 `IN_PROGRESS`。

REQ-036 已完成离线工程纵切：V10 增加免费教育来源、适用学段、栏目快照和幂等动作；服务端通过公共 HTTPS/DNS/同源/大小/超时门禁解析静态导航，成功刷新重新待批准，失败保留旧快照；Android 家长来源书架和儿童无 URL 栏目投影已接入。PostgreSQL 16.15 全量 42 项和 Android 两变体各 29 项通过。未知真实站点结构和目标平板尚未回放，因此 REQ-036/Stage 21 保持 `IN_PROGRESS`。
