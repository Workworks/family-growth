...
- 用户已授权 Agent 在既有需求、Stage 路线和安全边界内连续完成全部排期，并自行决定普通、可逆的产品与技术默认值；不得因为偏好型选项反复中断实施。
- 每项自主决策必须在目标 Stage Spec 或 AQ 中写明依据、取舍、回退方式和验证，不得只存在于会话记忆。
- 决策顺序固定为：儿童最佳利益与安全不变量 → 已登记用户原意 → 可验证的简单方案 → 最小数据/最低刺激/最少权限 → 可维护性与交付速度。
- 该授权不豁免真实验收，不授权侵权、真实金融、秘密泄露、破坏性操作或绕过 Android/第三方平台控制；需要真实设备、第三方许可、账号主体或部署环境才能完成的事项保留 `BLOCKED`，但不得阻塞其他可离线工程工作。
- 发布属于已登记总排期，可在代码、测试、签名、版本、远端摘要和公开资产门禁全部通过后执行；没有真机覆盖升级证据时只能声明“已发布”，不能声明“设备验收完成”。
