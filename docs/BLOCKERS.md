...
记录所有未达到 `COMPLETED` 的 Stage 及其缺少条件。**最后更新：2026-09-01。**

本清单区分两类：

- **外部阻塞**：缺少项目无法自行提供或未经授权影响共享环境的设备、仓库、凭据或第三方能力。代码可能已完成，但真实验收无法执行；不得用 Mock、构建或文档代替。
- **工程未完成**：能力在本项目范围内，只是尚未建设，继续按 Stage 开发即可；本轮最终审计后没有遗留的 Agent 可执行工程项。
...
已完成发布：Stage 26 能力及其后 Stage 27–32 深化已进入稳定签名 v0.3.9/15；workflow `33508682095` 成功，公开资产 12,065,826 字节，GitHub digest 与下载 SHA-256 `846fae8b...f3232eb` 一致，包名、版本与历史证书复验一致。
...
后续解除：Stage 17 已完成服务端冻结式零钱回收；Stage 26 已完成家长自助儿童数据导出/受约束删除、自动保留、睡眠时段、单次会话/强制休息和临时放行审计。Stage 10 的历史 AC-V1-11/13/17/19 仍记录当时证据，当前能力以 Stage 17/26 为准；剩余仅是真实设备/部署运行态验收。
...
状态：`BLOCKED`　文档：[Stage 20](stages/stage-20-report.md)

已完成：Flyway V9 四表、服务端年龄/推荐与覆盖学段事实源、家长配置和不可变审计、纪录片权利目录/审批/撤回/儿童安全投影、Android 本机兼容与远端同步、四套不同学段首页、受限动效/触觉；H2/PostgreSQL 16.15、Android 两变体、lint、OpenAPI 和文档门禁通过。

缺少：目标 Android 平板上的四学段横竖屏、物理触觉与关闭触觉、系统 Reduced Motion、TalkBack、重启持久化和真实服务同步回放。

解除条件：连接启用 USB 调试的 Android 8+ 平板/真机，并提供设备可访问的测试服务；也可由用户按 Stage 20 V20-05 场景自行测试并反馈可复现结果。

推进方：用户提供设备/服务或测试反馈；Agent 回放、修复并留证。

### Stage 22：幼儿园故事舞台与亲子现实活动

状态：`BLOCKED`　文档：[Stage 22](stages/stage-22-report.md)

已完成：目标/边界/依据/两遍设计和验收 Spec；Android 幼儿园单一行动安全投影；V12 服务端年龄带、五领域和发布聚合门禁；`KG-PACK-1.0.0` 十个原创亲子活动；家长年龄带/领域陪伴折页和一句行为观察。H2/PostgreSQL 16.15 与 Android 双变体离线门禁已通过。

缺少：目标平板上的年龄带/领域折页、布置、孩子离屏行动、家长观察回应、横竖屏、TalkBack、Reduced Motion 和触觉关闭回放。

解除条件：用户提供目标平板后执行 WP22-6 的触控、TalkBack、Reduced Motion、触觉关闭和完整亲子闭环回放。

推进方：用户提供设备；Agent 执行最终运行态验收。

## 2. 可离线工程已完成、仍受外部条件阻塞

以下 Stage 不再含可由 Agent 独立推进的工程工作；单独列出是为了保留各学段和历史 Stage 的设备验收入口，不表示仍需开发或发布。

### Stage 23：小学探索手册

状态：`BLOCKED`　文档：[Stage 23](stages/stage-23-report.md)

已完成：Android 低/高年级探索夹页、服务端小学分段、自动奖励、八个原创模板、求助/错因/再练和家长事实报告均通过 H2/PostgreSQL 16.15/Android 双变体离线门禁。

缺少：目标平板上的小学报告、横竖屏、TalkBack、大字体、Reduced Motion、稳定签名覆盖升级和数据保留回放；WP23-4/5/6 已进入 v0.3.7，发布不替代设备验收。

解除方式：能力已进入 v0.3.8；用户提供目标平板后由 Agent 回放运行态。发布成功不替代设备验收。

推进方：用户提供最终设备条件；Agent 回放并留证。

### Stage 24：初中学科实验台

状态：`BLOCKED`　文档：[Stage 24](stages/stage-24-report.md)

已完成：WP24-1–7 可离线范围，包括独立初中实验台、V16/V17 元数据与计划表、可审计顺序控制、六学科原创包、家长事实报告、H2/PostgreSQL 16.15 和 Android 双变体门禁。

缺少：目标平板横竖屏、大字体、TalkBack、Reduced Motion、真实触控排序、断网恢复和数据保留回放；可离线能力已进入 v0.3.7，发布不替代设备验收。

解除方式：能力已进入 v0.3.8；用户提供目标平板后由 Agent 按 V24-07 回放。发布成功不替代设备验收。

推进方：用户提供最终设备条件；Agent 回放并留证。

### Stage 25：高中自主学习室

状态：`BLOCKED`　文档：[Stage 25](stages/stage-25-report.md)

已完成：WP25-1–7 可离线范围，包括独立高中研究工作台、V18/V19 模块/目标/复盘/课节元数据、家长配置审计、六学科原创包、家长事实报告、H2/PostgreSQL 16.15 与 Android 双变体门禁。

缺少：目标平板横竖屏、大字体、TalkBack、Reduced Motion、目标冲突、断网恢复和数据保留回放；可离线能力已进入 v0.3.7，发布不替代设备验收。

解除方式：能力已进入 v0.3.8；用户提供目标平板后由 Agent 按 V25-07 回放。发布成功不替代设备验收。
...
已发布：REQ-045 的 v0.3.6/12 是 Stage 21 首个发布里程碑；该能力随后进入当前 latest v0.3.9/15，公开 digest、包名、版本和既有稳定证书均复验通过。远端发布不替代下述设备验收。
...
推进方：用户提供最终设备/服务后由 Agent 完成运行态回放；Stage 22–25 的学段深度内容可独立继续立项。
...
生产认证、任务审核、Wallet/Ledger、兑换、商店/储蓄/愿望、纯模拟基金、跨域报告、Stage 17 冻结式零钱回收、Stage 26 数据权利/自动保留/完整 App 内防沉迷和 Android 生产接入均已完成自动化范围；剩余是 Stage 8/10/26 真机与可信 HTTPS 总验收。

当前结论：REQ-020/REQ-022 已推进 Stage 3–10；Stage 3–7/9 在 PostgreSQL 通过；Stage 8/10 可离线工程与 Release 已过，缺平板真实登录/同步/总链路和覆盖升级回放。

解除方式：提供 Android 平板与可访问 HTTPS 服务完成运行态回放；LIM-012 已由 Stage 17 解除，LIM-013 已由 Stage 26 完成可离线工程与 PostgreSQL 证明，设备与外部备份边界继续真实保留。

推进方：Agent；涉及产品默认值、部署宿主和真实设备时由用户确认或提供条件。

### Stage 28：奖励预算、兑换额度与履约闭环

状态：`BLOCKED`　文档：[Stage 28](stages/stage-28-report.md)

已完成：V25 的版本化奖励预算/超限决策/精确覆盖、兑换方向和日月额度、儿童请求与家长审批、奖励现实履约；Stage 3/5 原子接入、并发额度桶、自由文本删除联动、Android 家庭约定夹和孩子中性状态均通过 H2、PostgreSQL 16.15、Android 双变体与文档门禁。

缺少：目标 Android 平板和设备可访问的可信 HTTPS 家庭服务，用于回放家庭约定配置、并发/断网后的状态恢复、孩子待回应、家长批准/拒绝、现实兑现、TalkBack、旋转、大字体和重启保留。

解除方式：用户提供设备/服务后由 Agent 执行 V28-08 并留真实运行态证据。

### Stage 29：家庭成员、设备配对与最小待办

状态：`BLOCKED`　文档：[Stage 29](stages/stage-29-report.md)

已完成：V26 的 OWNER/GUARDIAN、一次性邀请、独立 PIN、成员撤销，PARENT/CHILD 一次性设备配对、设备绑定会话/撤销、多孩子显式切换，以及五类低刺激家长待办；Android 默认配对码连接、成员/设备管理和数据删除去标识化联动已通过 H2、PostgreSQL 16.15、双变体与文档门禁。

缺少：两台目标 Android 设备和设备可访问的可信 HTTPS 家庭服务，用于回放真实家长/孩子配对、并发代码消费、设备/成员撤销即时 401、重启、旋转、TalkBack、大字体和不请求儿童系统通知。
...
缺少：目标平板从稳定 v0.3.8 同签名覆盖升级至 v0.3.9 后的数据保留、杀进程重启、断网恢复，以及两台设备对同一学习事实制造 409 后的家长处理体验。
...
状态：Stage 27–32 的 Agent 可执行工程、治理和稳定 v0.3.9 发布均已完成；各 Stage 仅因目标设备/可信服务运行态保持 `BLOCKED`。

## 4. 当前已知限制

- 现有 v0.2.0–v0.3.9 APK 使用同一稳定 release 身份；历史 debug APK 仅供内部证据，不能与 release 包互相覆盖。
- Stage 3–10、17、26 的生产 API 与业务闭环已完成自动化范围；家庭局域网正式部署仍需设备可访问的可信 TLS、备份恢复运行态和目标平板 E2E。
- Stage 32 已将工具链固定为 AGP 8.13.2 / Gradle 8.13 / JDK 17，compileSdk 36 兼容警告已解除；targetSdk 34 暂不随工具链升级改变平台行为。
