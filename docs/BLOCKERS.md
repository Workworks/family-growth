...
缺少：真实平板桌面的系统蒙版/不同壁纸对比；v0.3.1 实际应用内下载需发布一个更高测试版本才能完整回放。旧 v0.3.0 下载端含 BUG-002，应先手动安装 v0.3.1。

解除条件：连接启用 USB 调试的 Android 平板/真机，手动安装 v0.3.1 并验证图标；随后发布同签名更高 versionCode 版本，使用 v0.3.1 在 App 内完成下载、SHA-256 校验、系统安装和数据保留。
