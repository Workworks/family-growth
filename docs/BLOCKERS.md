...
记录所有未达到 `COMPLETED` 的 Stage 及其缺少条件。**最后更新：2026-08-28。**
...
当前工作：孩子“我的任务/自主学习”、五级教材选择、受限官方 WebView、V10 家长免费来源/动态栏目/儿童无 URL 投影，以及 V11 不可变课程版本、九类活动、题目/答案隔离、Assignment/Attempt/Completion/MasteryEvidence、生产 API 和 Android 学习路径均已通过离线门禁。工程仍需 V12 持久化 outbox、进程重启、专门 401/409 冲突恢复和家长 Android 可视化建课，之后才进入各学段深度内容。

外部验收：需要可访问家庭服务和目标平板验证真实免费来源的栏目兼容、成功刷新后重新批准、失败保留快照、儿童动态目录、横竖屏和 TalkBack；还需验证国家智慧教育平台加载/播放/返回。公共 DNS 预检必须配合部署侧出站防火墙/代理阻断内网和云元数据。共用教学引擎完成后继续回放多活动、断网重启、401/409 和家长/孩子 E2E。若未来要求抽取视频直链、离线缓存、改编页面或自动回传完成证据，还需要平台许可或官方 SDK/API。
