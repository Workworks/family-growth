...
缺少：真实平板桌面的系统蒙版/不同壁纸对比；v0.3.2 已作为更高测试版本发布，可用于 v0.3.1 实际应用内下载回放。旧 v0.3.0 下载端含 BUG-002，应先手动安装 v0.3.1。

解除条件：连接启用 USB 调试的 Android 平板/真机，手动安装 v0.3.1 并验证图标；再使用 v0.3.1 在 App 内更新至已发布的 v0.3.2，完成下载、SHA-256 校验、系统安装和数据保留。
...
缺少：真实平板上的横竖屏排版、视频硬件/软件解码与计时、兴趣和观看进度重启保留、TalkBack，以及 v0.3.1→v0.3.2 应用内覆盖升级。
...
- 现有 v0.2.0–v0.3.2 APK 使用同一稳定 release 身份；历史 debug APK 仅供内部证据，不能与 release 包互相覆盖。
