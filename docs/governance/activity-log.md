...
## 2026-08-30 · Activity 68 · 全排期连续交付授权与执行重排

- 用户要求一次性完成全部排期，并授权 Agent 自主决定方向、不再逐项询问；登记 REQ-050/P0，并把授权边界固化到 `codex-skills.md` 2c。
- 决定保持既有 Stage 23→24→25→26 依赖顺序：先完成小学家长事实报告，再分别建设初中、高中深度体验，最后收口跨学段、数据权利、完整防沉迷、Android 生产接入和发布门禁。
- 普通可逆默认值由 Agent 选择并在对应 Spec/AQ 留痕；儿童安全、账本、隐私、真实金融、Android 安装和第三方授权红线不因连续授权而降低。
- 目标平板、可访问家庭服务与第三方许可仍是外部验收条件；其缺失只保留真实阻塞，不中断可离线实现。

## 2026-08-30 · Activity 69 · Stage 23 小学家长事实报告离线收口

- 新增家长专属小学事实报告：聚合历史小学 Assignment 状态、未回应求助/错误、未完成及到期再练，并只统计最近 7 天明确 `LEARNING` 使用分钟。
- API 对孩子返回 403、跨家庭返回 404，响应不含答案键、家长私密说明、具体答案、评分或能力推断；切换学段后仍保留历史小学事实。
- Android 报告进入既有家长学习回应区，不增加儿童导航；按学科展示完成、待回应、求助和到期再练，并明确不是成绩评价。
- H2 全量 24 个领域、2 个基础设施和 26 个 Boot 测试通过（6 个 PostgreSQL 条件跳过）；PostgreSQL 16.15 schema/API 共 3 项通过；Android debug/release JVM、lint、assemble 111 项通过。一次性数据库容器已停止并自动删除，目标平板仍未执行。

## 2026-08-30 · Activity 70 · Stage 24 初中学科实验台立项

- Stage 23 提交 `636305d` 已推送 main；可离线范围完成，目标平板项保持真实阻塞。
- 按 REQ-050 连续进入 Stage 24，在实现前固化初中章节/知识点、可调整计划、过程证据、原创模板、家长事实报告及权限/并发/设备门禁。
- 前端两遍设计采用安静的“实验记录夹”：学科轨道、当前实验页和真实证据轨道；第二遍移除拟物试管、霓虹、徽章和通用仪表盘，只保留证据黄当前节点。排序必须同时提供上移/下移，不依赖拖拽或动效。

## 2026-08-30 · Activity 71 · Stage 24 Android 实验台首切

- 初中动态课程不再落入初高中共用通用卡片：新增学科轨道、章节/课节标题、问题/证据/解释真实节点、单一当前证据操作、求助和 25 分钟以内中性休息提示。
- 视觉按 Stage 24 Spec 使用夜航蓝、测量青、证据黄和纸页灰；未加入拟物实验器材、排名、百分比、倒计时、连续动画或新增儿童导航。
- 没有服务端排序事实前不做本机伪排序；界面明确说明待 WP24-4 持久化后开放调整。Android debug JVM、lint、assemble 53 项通过；release 与目标平板未执行。

## 2026-08-31 · Activity 72 · Stage 24 初中生产闭环离线收口

- Flyway V16/V17 新增按孩子隔离的初中学习计划、不可变移动审计和课节章节/知识点/学习目标/安全说明；发布校验拒绝超过 25 分钟、元数据缺失、跨学段携带及高风险家庭实验指导。
- 排序只投影未开始的初中自主 Assignment；服务端锁定计划 revision、验证幂等载荷并交换最近可排序项。进行中项会被跨过而不被移动，跨家庭访问隐藏为 404，重复请求不重复写审计。
- Android “实验记录夹”接入真实服务端计划、上移/下移、章节目标和安全边界；`JUNIOR-PACK-1.0.0` 提供语文、数学、英语、物理、生物、历史六个原创现实活动，只保存草稿。家长报告只展示最近 7 天的计划、状态、求助、再练与明确学习分钟。
- H2 全量 25 个领域、2 个基础设施和 28 个 Boot 测试通过（6 个 PostgreSQL 条件跳过）；PostgreSQL 16.15 验证 17 个迁移、64 张生产表和 Stage 23/24 API，并在两个并发移动中只提交一个 revision。Android debug/release JVM、lint、assemble 通过。一次性数据库容器已停止并自动删除，目标平板如实保留阻塞。

## 2026-08-31 · Activity 73 · Stage 25 高中自主学习室立项

- Stage 24 提交 `2aa10bc` 已推送 main；可离线范围完成，目标平板项保持真实阻塞。
- 按 REQ-050 连续进入 Stage 25，在实现前固化高中模块配置、本周目标、专题证据、不可覆盖复盘、原创模板、家长事实报告及权限/并发/设备门禁。
- 前端两遍设计采用安静“研究工作室”：模块索引、本周研究板、证据与复盘；第二遍移除成绩仪表盘、红黄风险灯、时长环、连续计时、徽章和便利贴堆叠。目标、复盘和投入时长不生成 MASTERED 或奖励。

## 2026-08-31 · Activity 74 · Stage 25 高中生产闭环离线收口

- Flyway V18/V19 新增高中模块配置/选择/审计、周目标/动作、不可覆盖复盘和高中课节元数据共 7 张表；家长配置模块边界，孩子只能在本人已启用模块内建立、更新和归档目标。
- 高中课节随不可变版本保存模块类型、专题、研究问题、预期证据和安全边界；发布拒绝缺失元数据、超过 45 分钟、高风险指导和跨学段元数据。目标与复盘不写 Mastery、Wallet 或 Ledger。
- Android 新增安静的“自主学习室”、本周目标、证据行动、策略复盘、主动求助、家长模块配置和事实报告；`SENIOR-PACK-1.0.0` 提供语文、数学、英语、物理、历史、信息技术六个原创研究活动，只保存草稿。
- H2 全量 26 个领域、2 个基础设施和 30 个 Boot 测试通过（6 个 PostgreSQL 条件跳过）；PostgreSQL 16.15 验证 19 条迁移、71 张生产表和 Stage 23–25 共 7 项测试且零跳过。Android debug/release JVM、lint、assemble 全部通过。一次性数据库容器已停止并自动删除；目标平板如实保留阻塞，主线进入 Stage 26。

## 2026-08-31 · Activity 75 · Stage 26 连续性、隐私与防沉迷离线收口

- Flyway V20–V22 增加睡眠时段、使用策略动作、临时放行、儿童数据请求/动作和学段迁移审计；有效学段变化只归档旧学段未开始的自主 Assignment，并恢复目标学段曾归档项，所有历史学习、奖励和账本事实保留。
- 家长可生成稳定 schema 的儿童 JSON 导出；删除采用十分钟一次性 token + 服务端 PIN 两阶段确认，去除直接标识、自由文本、愿望标题、非财务使用明细、临时放行和儿童会话，保留 Ledger、必要审计、幂等与最小学习状态。
- 防沉迷由服务端统一判定默认 21:30–06:30 睡眠窗、日上限和最多 60 分钟家长临时放行；Android 增加隐私中心、导出文件、删除预览/PIN 确认、学段切换预览和温和离屏锁定页。前端继续保持分学段结构、儿童三入口、无倒计时/随机奖励/羞辱反馈。
- H2 全量 26 个领域、2 个基础设施和 32 个 Boot 测试通过（6 个 PostgreSQL 条件跳过）；PostgreSQL 16.15 验证 22 条迁移、76 张生产表和 Stage 20/23/25/26 关键链共 9 项且零跳过，补强删除策略后再次通过 H2/PostgreSQL。Android 双变体 111 个 Gradle 任务通过；目标平板与 HTTPS 仍如实阻塞，下一步发布稳定 Release。

## 2026-08-31 · Activity 76 · 发布并复验 v0.3.7
...
## 2026-08-31 · Activity 83 · Stage 28 奖励治理立项

- 建立 P-04/05/07 的 Stage 28 Spec：奖励预算以已批准 Completion 为事实，超限只能按版本规则固定转换或先由家长创建精确一次性覆盖；实际奖励与 Ledger 必须原子一致。
- 兑换控制增加方向开关、日/月额度和可配置儿童审批；待审批不扣账，家长批准后才复用 Stage 5 原子兑换。奖励批准与现实履约分离，只有家长可进入 `FULFILLED` 并记录备注。
- Android 使用低刺激“家庭约定夹”，孩子只看等待回应/已兑现，不接收预算原因、费率压力或损失语言。目标平板运行态独立保留 V28-08。

## 2026-08-31 · Activity 84 · Stage 28 可离线闭环与并发修正

- V25 新增版本化奖励预算、一次性覆盖、不可变决策、兑换控制/审批、日月额度占用和履约动作；Stage 3 审核在同一事务裁决实际奖励，Stage 5 确认在同一事务占用额度并写 Wallet/Ledger。
- 初版“锁规则后汇总订单”被新增并发测试证明仍可能让两个确认同时穿过额度。修正为按孩子/方向/日月周期初始化额度桶，并用带上限条件的数据库原子更新依次占用；第二个并发事务只能得到 409，后续钱包/流水失败则额度占用随事务回滚。
