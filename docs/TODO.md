...
| P0 | 14 | `IN_PROGRESS` | 审计公开范围，按 Stage commit，创建并推送 `Workworks/family-growth`；随后配置稳定签名 Release | Agent；签名备份需用户确认 | Stage 14 报告与 `evidence/stage-14/acceptance.json` |
...
| 顺序 | Stage | 主题 | 前置条件 |
| ---: | ---: | --- | --- |
| 1 | 3–7 | 后端深度业务闭环 | 按用户当前要求后移；恢复时先更新对应 Stage Spec |
