...
| P1 | 32 | `BLOCKED` | Agent 可执行工程、治理和 v0.3.9 发布已完成；连接目标平板/可信 HTTPS 回放同签名升级、加密数据保留、无障碍和两设备冲突 | 用户提供设备/服务；Agent 回放 | Stage 32 报告、v0.3.9 Release、目标设备证据 |
...
| P0 | 26 | `BLOCKED` | WP26-1–13 已完成并进入 latest v0.3.9/15；连接目标平板与可信 HTTPS 服务回放首装/覆盖升级、四学段、TalkBack/旋转/大字体/Reduced Motion、触觉、断网和数据保留 | 用户提供设备/服务；Agent 回放 | Stage 26/32 报告、v0.3.9 Release |
...
| P0 | 11 | `BLOCKED` | 稳定 Release 链已完成且 latest 为 v0.3.9；连接 Android 设备回放 App 内检查、下载、校验、系统确认和数据保留 | 用户提供设备；Agent 回放 | 真机覆盖升级证据 |

## 2. 可由 Agent 继续推进

Stage 27–32 的 Agent 可执行工程、最终治理和 v0.3.9 稳定发布均已完成。当前没有可在缺少设备/服务的条件下继续闭环的工程项；真实平板与可信 HTTPS 是剩余独立外部验收。完整拆分见 [教学与项目剩余事项清单](requirements/teaching-and-project-backlog.md)。
...
| P0 | 11/14 | 启用 USB 调试的 Android 真机/平板；仓库、稳定签名和 latest v0.3.9 Release 已具备 |
