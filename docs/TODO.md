...
| P0 | 19 | `BLOCKED` | 手动安装 v0.3.3 后在应用内更新到 v0.3.4，反馈显示阶段、结果和数据保留 | 用户测试；Agent 根据反馈修复 | Stage 19 真机证据 |
...
| 顺序 | Stage | 主题 | 前置条件 |
| ---: | ---: | --- | --- |
| 1 | 17 | 服务端零钱回收与冻结式兑现闭环 | Stage 19 可离线修复已交付；不接真实支付 |
...
| P0 | 19 | Android 设备手动安装 v0.3.3，再应用内更新到 v0.3.4；反馈排队/连接/下载/校验或具体失败阶段 |
