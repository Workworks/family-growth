...
| P0 | 21 | `IN_PROGRESS` | 实现 V10、不可变课程版本、九类活动、Assignment/Attempt/Completion/Evidence、生产 API 和 Android 真实课程接入 | Agent | `stages/stage-21-report.md`、`evidence/stage-21/acceptance.json` |
...
Stage 20 可离线工程与门禁已经收口，只剩目标平板验收。Stage 21 Spec 已建立，Agent 正在建设 Course/Unit/Lesson/Activity、题库、分配/尝试/完成/复做、证据与 Android 生产接入；LIM-013 数据权利/完整时段审计仍为并行 P0 安全事项。完整拆分见 [教学与项目剩余事项清单](requirements/teaching-and-project-backlog.md)。
