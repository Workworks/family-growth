...
| P1 | 24 | `IN_PROGRESS` | WP24-2：实现独立初中学科实验台、真实 Assignment 状态投影、可访问顺序控制和休息出口 | Agent | Stage 24 Spec、Android 双变体与 `evidence/stage-24/` |
...
Stage 20–23 的可离线工程与门禁已经收口，只剩目标平板/服务验收。按 REQ-050 不再因普通产品偏好暂停：当前执行 Stage 24 WP24-2，随后连续进入 Stage 25、26；LIM-013 数据权利、完整 App 内防沉迷和 Android 生产接入在 Stage 26 作为 P0 收口。完整拆分见 [教学与项目剩余事项清单](requirements/teaching-and-project-backlog.md)。
