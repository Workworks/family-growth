...
public interface FamilyGrowthStore {
 ...
 Optional<Family> findFamily(UUID id); Optional<ParentProfile> findParent(UUID id); Optional<ChildProfile> findChild(UUID id); Optional<GrowthPlan> findPlan(UUID id); Optional<GrowthGoal> findGoal(UUID id); Optional<GrowthTask> findTask(UUID id);
}
